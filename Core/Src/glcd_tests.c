...
void glcd_TestBorder() {
    ...
    uint8_t y = 0;
    do {
        glcd_DrawPixel(x, y, BLACK);

        if((y == 0) && (x < 127))
            x++;
        else if((x == 127) && (y < (LCD_HEIGHT-1)))
            y++;
        else if((y == (LCD_HEIGHT-1)) && (x > 0)) 
            x--;
        else
            y--;

        glcd_DrawPixel(x, y, WHITE);
        glcd_UpdateScreen();
...
void glcd_TestFonts() {
    ...
    glcd_SetCursor(2, y);
    glcd_WriteString("Font 16x26", Font_16x26, WHITE);
    y += 26;
    #endif
    ...
    ssd1306_SetCursor(2, y);
    ssd1306_WriteString("Font 8x5", Font_8x5, WHITE);
    #endif
...
//    HAL_Delay(3000);
//    ssd1306_SetCursor(2, 2);
//    ssd1306_WriteString("TestBorder", Font_7x10, WHITE);
//    ssd1306_UpdateScreen();
//    HAL_Delay(1000);

/*this works but take much time
	glcd_Fill(BLACK);
    glcd_TestBorder();
    HAL_Delay(1000);
*/
